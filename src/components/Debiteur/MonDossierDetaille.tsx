...
const MonDossierDetaille: React.FC = () => {
  const { user } = useAuth();
  const { clients, invoices, payments, communications, refreshData } = useCrm();
  const [activeTab, setActiveTab] = useState('overview');
  const [isLoading, setIsLoading] = useState(false);
  const [debiteurDetails, setDebiteurDetails] = useState<any>(null);
  const [showPaymentModal, setShowPaymentModal] = useState(false);

  // Trouver le débiteur correspondant à l'utilisateur connecté
  ...
  useEffect(() => {
    const loadDebiteurDetails = async () => {
      if (!debiteurData) return;
      
      try {
        setIsLoading(true);
        
        // Get related data from context
        const debiteurInvoices = invoices.filter(inv => inv.client_id === debiteurData.id);
        const debiteurPayments = payments.filter(payment => payment.client_id === debiteurData.id);
        const debiteurCommunications = communications.filter(comm => comm.client_id === debiteurData.id);
        
        setDebiteurDetails({
          ...debiteurData,
          invoices: debiteurInvoices,
          payments: debiteurPayments,
    ...
    loadDebiteurDetails();
  }, [debiteurData, invoices, payments, communications]);
  ...
  const handleRefresh = async () => {
    ...
    if (debiteurData) {
      try {
        ...
        console.error('Error refreshing debiteur details:', error);
      }
    }
  ...
  // Use debiteur details if available
  const debiteurInvoices = debiteurDetails?.invoices || [];
  const debiteurPayments = debiteurDetails?.payments || [];
  const debiteurCommunications = debiteurDetails?.communications || [];

  // Calculate totals
  const totalAmount = debiteurData.total_amount;
  const totalOriginal = debiteurInvoices.reduce((sum: number, inv: any) => sum + inv.original_amount, 0);
  const totalPaid = debiteurInvoices.reduce((sum: number, inv: any) => sum + (inv.paid_amount || 0), 0);
  const overdueInvoices = debiteurInvoices.filter((inv: any) => inv.status === 'overdue').length;
  const recoveryRate = totalOriginal > 0 ? Math.round((totalPaid / totalOriginal) * 100) : 0;

  // Get manager info
  const managerName = debiteurData.manager_id === "manager1" ? "Marie Dubois" : 
                      debiteurData.manager_id === "manager2" ? "Pierre Martin" :
                      debiteurData.manager_id === "manager3" ? "Sophie Leroy" : "Jean Robert";
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h1 className="text-2xl font-bold text-gray-900">Mon Dossier Détaillé</h1>
              <p className="text-gray-600">{debiteurData.name}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {debiteurData.company && (
            <div className="flex items-center">
              <Building className="h-5 w-5 text-gray-400 mr-3" />
              <div>
                <p className="text-sm font-medium text-gray-700">Entreprise</p>
                <p className="text-gray-900">{debiteurData.company}</p>
              </div>
            </div>
          )}
          
          {debiteurData.address && (
            <div className="flex items-center">
              <MapPin className="h-5 w-5 text-gray-400 mr-3" />
              <div>
                <p className="text-sm font-medium text-gray-700">Adresse</p>
                <p className="text-gray-900">{debiteurData.address}</p>
              </div>
            </div>
          )}
          ...
          {debiteurData.siret && (
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-900">{debiteurData.siret}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">Gestionnaire</p>
          ...
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold text-purple-900">{recoveryRate}%</p>
              </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-sm text-gray-600 mb-2">
            ...
            <span>{recoveryRate}% complété</span>
          </div>
        ...
        <div className="p-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-blue-700">18 décembre 2024 - {managerName}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {debiteurInvoices.map((invoice: any) => {
                        ...
                        return (
                          <tr key={invoice.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm text-red-600">{daysOverdue} jours</td>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Votre Gestionnaire</h3>
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                              ...
                              {daysOverdue > 0 && (
                                <span className="text-red-600">Retard: {daysOverdue} jours</span>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-right">
                          ...
                          {invoice.status === 'partial' && (
                            <div className="text-sm text-gray-500">
                              sur {formatCurrency(invoice.original_amount)}
                            </div>
                          )}
          ...
          {activeTab === 'communications' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {debiteurCommunications.map((comm: any) => {
                    ...
                    <div key={comm.id} className="bg-white border border-gray-200 rounded-lg p-4">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 mt-1 whitespace-pre-line">{comm.content}</p>
                ...
                {debiteurCommunications.length === 0 && (
...
export default MonDossierDetaille;