  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        ...
        if (debiteur) {
          // Get debiteur communications
          const debiteurCommunications = communications.filter(comm => comm.client_id === debiteur.id);
          
          // Get debiteur payments
          const debiteurPayments = payments.filter(payment => payment.client_id === debiteur.id);
          
          // Convert to history events
          const events: HistoryEvent[] = [];
          
          // Add communications to history
          debiteurCommunications.forEach(comm => {
          ...
          debiteurPayments.forEach(payment => {
          ...
          if (debiteur.status === 'critical' || debiteur.status === 'orange') {
            const statusDate = new Date(debiteur.updated_at);
            events.push({
              id: `status_${debiteur.id}`,
              type: 'status_change',
              title: `Dossier passé en statut ${debiteur.status}`,
              ...
              impact: 'high',
              details: {
                previousStatus: debiteur.status === 'critical' ? 'orange' : 'yellow',
                newStatus: debiteur.status,
