...
const InvoicesPage: React.FC = () => {
  ...
  const [debiteurInvoices, setDebiteurInvoices] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadInvoices = async () => {
      ...
      try {
        ...
        if (debiteur) {
          // Get debiteur invoices
          const userInvoices = invoices.filter(invoice => invoice.client_id === debiteur.id);
          ...
          setDebiteurInvoices(formattedInvoices);
        }
      } catch (error) {
  ...
  const filteredInvoices = debiteurInvoices.filter(invoice => {
  ...
  const totalAmount = debiteurInvoices.reduce((sum, inv) => sum + inv.amount, 0);
  const totalPaid = debiteurInvoices.reduce((sum, inv) => sum + inv.paidAmount, 0);
  const overdueCount = debiteurInvoices.filter(inv => inv.status === 'overdue').length;
  const paidCount = debiteurInvoices.filter(inv => inv.status === 'paid').length;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{paidCount}/{debiteurInvoices.length}</div>
